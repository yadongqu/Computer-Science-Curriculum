...
public class PixImage {
  ...
  public PixImage(int width, int height) {
    ...
    for (int i = 0; i < width; i++){
      for (int j = 0; j < height; j++){
        this.colors[i][j] = new Color((short)0,(short)0,(short)0);
      }
    }
  }
  ...
  public void setPixel(int x, int y, short red, short green, short blue) {
    // Your solution here.
    this.colors[x][y] = new Color(red, green, blue);
  }
  ...
  public Color getAverage(int x, int y){
    int neighbors = (short) 1;
    Color sum = this.colors[x][y];
    if (x - 1 >= 0 ){
      sum.add(this.colors[x-1][y]);
      neighbors += 1;
      if(y - 1 >= 0){
        sum.add(this.colors[x-1][y-1]);
        neighbors += 1;
      }
