...
def find_closest(location, centroids):
    ...
    # BEGIN Question 3
    "*** REPLACE THIS LINE ***"
    return min(centroids, key=lambda x: (x[0]-location[0]) ** 2 \
             + (x[1]-location[1])**2)
    # END Question 3
...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        temp.append([find_closest(restaurant_location(restaurant), centroids), restaurant])
...
def find_predictor(user, restaurants, feature_fn):
    ...
    sxy = sum([(n[0] - mean(xs))*(n[1] - mean(ys)) for n in zip(xs,ys)])
    b = sxy / sxx
    a = mean(ys) - b * mean(xs)
